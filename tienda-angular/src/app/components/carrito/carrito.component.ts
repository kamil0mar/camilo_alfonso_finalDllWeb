...
export class CarritoComponent implements OnInit {
  ...
  pagarCarrito(){
    ...
    this.http.get('https://tienda-angular2.firebaseio.com/productos/.json').map((response : Response) => {
        this.catalogo; //Asignar el valor obtenido en la consulta al arreglo de objetos catalogo
      }
    ).subscribe(
      ()=>{
  ...
  actualizarDisponible(id:number, itemCatalogo:Producto){
    ...
    .map((response : Response) => {
        return this.catalogo;
         //Asignar a la lista de productos actuales el valor actualizado de los productos en la base de datos
      }
    )
  }
